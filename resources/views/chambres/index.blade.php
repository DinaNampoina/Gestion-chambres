    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('chambres.create') }}"
                class="px-4 py-2 bg-brand-600 hover:bg-brand-700 text-white rounded-md text-sm">+ Ajouter une chambre</a>
        </div>
    </x-slot>
            ...
            <div class="bg-white dark:bg-brand-900 shadow-sm rounded-lg overflow-hidden">
                <table class="w-full text-sm table-fixed">
                    <thead class="bg-brand-50 dark:bg-brand-800">
                        <tr>
                            <th
                                class="w-1/6 px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase">
                                Numéro</th>
                            <th
                                class="w-1/6 px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase">
                                Type</th>
                            <th
                                class="w-1/6 px-4 py-3 text-center text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase">
                                Capacité</th>
                            <th
                                class="w-1/6 px-4 py-3 text-center text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase">
                                Prix/nuit</th>
                            <th
                                ...
                                Statut</th>
                            <th
                                class="w-1/6 px-4 py-3 text-right text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase">
                                Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-brand-100 dark:divide-brand-800">
                        @foreach ($chambres as $chambre)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-center">{{ number_format($chambre->prix, 0, ',', ' ') }} Ar
                                </td>
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="flex justify-end gap-3">
                                        <a href="{{ route('chambres.edit', $chambre) }}"
                                            class="text-accent-700 dark:text-accent-400 hover:underline">Modifier</a>
